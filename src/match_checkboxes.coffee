...
#
module.exports.matchCheckboxes = (formData, formSchema, checkboxes, words, schemaToPage, schemaToData, image) ->
	checkboxFields = formSchema.filter((field) -> field.type is 'checkbox')

	logImage = new dv.Image(image).toColor()
	# Remove all boxes which mapped to somewhere inside a word of length > 1
	#longWords = (word.box for word in words when word.text.length > 1)
	#checkboxes = checkboxes.filter (check) -> not longWords.some((word) -> mostlyWithin(check.box, word))
	...
	for field in checkboxFields
		...
		console.log field.path, closeWord
